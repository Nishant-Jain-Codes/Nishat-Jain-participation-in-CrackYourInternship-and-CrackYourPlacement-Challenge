...
public:
    TreeNode* buildTree(vector<int>& inorder, vector<int>& postorder) {
        return builder(postorder, 0, postorder.size() - 1, inorder, 0,
                       inorder.size() - 1);
    }

private:
    TreeNode* builder(vector<int>& post, int pS, int pE, vector<int>& ino,
                      int iS, int iE) {
        if (pS > pE)
            return NULL;
        if (pS == pE)
        ...
        // cout<<rootIndexInInorder<<" "<<ino[rootIndexInInorder]<<"
        // "<<rootVal<<endl;
        int iSL = iS;
